...
const PairedSampleTtest = ({ data, headers }) => {
  ...
  // Handler for selecting paired groups
  ...
  // Function to perform the paired t-test by calling the backend
  const doPairedSampleTtest = async () => {
    if (!group1 || !group2) {
      alert("Please select both groups for the paired t-test.");
      return;
    }
    ...
    try {
      ...
      const response = await fetch('http://127.0.0.1:5000/api/paired-sample-ttest', {
        method: 'POST',
        headers: {
  ...
  return (
    <div>
      <h1>Paired Sample T-test - Frontend UI</h1>
      
      {/* Dropdowns to select paired groups */}
      ...
      <button onClick={doPairedSampleTtest}>Paired Sample T-test</button>
...
export default PairedSampleTtest;
